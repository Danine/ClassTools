...
def rename(path):
	...
	for filename in files:
		...
		if portion[1] == '.docx' or portion[1] == '.doc' or portion[1] == '.pdf':#只处理docx和doc格式
			tempID = re.match('.*(201[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]).*',portion[0])
			ID = tempID.group(1)
			IDs.append(ID)
			newname = path + ID + '_' + mp[ID] + u'_毛概总结' + portion[1]#新的文件名，每次不同
			os.rename(oldname, newname)
	return ID

def notfound(ID,IDs):#查找还没有交作业的同学
	for ID in mp:#遍历所有学号
